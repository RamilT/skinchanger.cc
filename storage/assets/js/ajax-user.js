/*$(function() {
    var socket = io.connect('wss://vds0.skinchanger.cc:3337');
    ...
    $.ajax({
        ...
        success: function (result) {
            if (!result.status) {
                ...
                n.update({'type': 'danger', 'message': result.message, 'title': '<strong>Error</strong>'});
            } else {
                $('.modal_login').modal('hide');
                window.location.href=redirect_after_auth?redirect_after_auth:'/cabinet';
            }
        }
    });
}
...
function settings_cancelRecurring() {
    ...
    $.ajax({
        ...
        dataType: "json",
        success: function (result) {
            if(result.status) n('Успешно!', 'Автопродление отключено', 'success');
            else n('Ошибка!', 'Согласно пункту 5.2. пользовательского соглашения отменить подписку вы можете спустя 40 часов после её получения', 'danger');
...
function settings_cancelRecurring2() {$('.modal_cancel').modal('hide');}